...
-- 3) Base users table (managed by custom auth backend)
-- This table stores core user data independent of Supabase Auth
...
-- 4) User profiles (links to public.users)
-- user_profiles stores app-specific metadata and role information
...
-- Note: these policies assume JWT-authenticated users where auth.uid() returns the user's UUID.
-- Allow users to insert their profile and only modify their own profile; admins can view others.
ALTER TABLE public.user_profiles ENABLE ROW LEVEL SECURITY;

-- Insert: allow authenticated users to insert a profile record for themselves
...
DROP POLICY IF EXISTS user_profiles_modify_owner_or_admin_delete ON public.user_profiles;
CREATE POLICY user_profiles_modify_owner_or_admin_delete
...
-- Update: owners may update their job (e.g., status), admins may update all
...
-- Inference results RLS: allow owners (via job) or admins
...
DROP POLICY IF EXISTS model_analytics_admin_update ON public.model_analytics;
CREATE POLICY model_analytics_admin_update
  ON public.model_analytics FOR UPDATE
  USING (
    auth.uid() IS NOT NULL
    AND EXISTS (SELECT 1 FROM public.user_profiles up WHERE up.id = auth.uid() AND up.role IN ('admin','superadmin'))
  )
...
DROP POLICY IF EXISTS model_analytics_admin_delete ON public.model_analytics;
CREATE POLICY model_analytics_admin_delete
  ON public.model_analytics FOR DELETE
  USING (
    auth.uid() IS NOT NULL
    AND EXISTS (SELECT 1 FROM public.user_profiles up WHERE up.id = auth.uid() AND up.role IN ('admin','superadmin'))
  );
...
-- 10) Storage buckets
-- Create a bucket for waveform blobs. The storage extension provides storage.create_bucket in Supabase.
-- If the function is not available in SQL environment, create the bucket via Supabase Dashboard > Storage.
DO $$
BEGIN
  -- Attempt to create 'waveforms' bucket; if the storage extension exists this will succeed, otherwise fail silently.
  BEGIN
...
-- 11) Example: grant execute on set_updated_at to public (optional)
